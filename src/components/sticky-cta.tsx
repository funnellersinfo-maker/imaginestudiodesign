...
export default function StickyCTA({ onQuoteClick }: StickyCTAProps) {
  ...
  const handleQuote = useCallback((e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
    e.stopPropagation();
    onQuoteClick();
  }, [onQuoteClick]);
...
export function FloatingCTA({ onQuoteClick }: { onQuoteClick: () => void }) {
  ...
  return (
    <div
      className="hidden md:flex fixed bottom-6 right-6 z-50 flex-col gap-3 transition-opacity duration-300"
